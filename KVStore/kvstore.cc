...
std::string getTabletPath() {
    ...
    tabletFile.open(tabletPath, std::fstream::in | std::fstream::out | std::fstream::app);
    if (!tabletFile) {
        tabletFile.open(tabletPath, std::fstream::in | std::fstream::out | std::fstream::app);
    }
    tabletFile.close();
...
int writeTablet(tablet_struct *tablet) {
    ...
    std::fstream tabletFile;
    tabletFile.open(path, std::fstream::in | std::fstream::out | std::fstream::trunc);
    if (!tabletFile) {
        tabletFile.open(path, std::fstream::in | std::fstream::out | std::fstream::trunc);
    }
    // close file
    ...
    /* write to file */
    std::unordered_map<std::string, std::unordered_map<std::string, std::string>> map = tablet->map;
